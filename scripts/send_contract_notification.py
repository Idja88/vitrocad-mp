...
def send_contract_notification():
    ...
    if contract_type_id_list:
        contract_type_id_list = [
            ctid.strip() for ctid in contract_type_id_list.split(",") if ctid.strip()
        ]
    else:
        contract_type_id_list = []

    contract_type_id_list_str = ", ".join(
        [f'Guid("{id}")' for id in contract_type_id_list]
    )
    ...
    try:
        ...
        for contract in contracts:
            ...
            contract_holder = client.get_mp_item(
                contract.get("fieldValueMap", {}).get("contract_holder").get("id")
            )
            contract_holder_email = contract_holder.get("fieldValueMap", {}).get(
                "email"
            )
            ...
            if result:
                print(
                    f"Email sent to {contract_holder_email} for contract {contract_id}"
                )
            else:
                print(
                    f"Failed to send email to {contract_holder_email} for contract {contract_id}"
                )
